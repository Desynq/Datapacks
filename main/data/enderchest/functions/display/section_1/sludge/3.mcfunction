execute in overworld run item replace block 0 0 1 container.0 with apple{Tags:["custom"],type:stat_health,display:{Name:'{"italic":"false","color":"red","text":"Max Health (+4)"}',Lore:['""']}}
	tag @s add temp
	...
	tag @s remove temp
	execute in overworld run item replace entity @s enderchest.3 from block 0 0 1 container.0
