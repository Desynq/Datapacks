...
execute at @e[tag=refinery,scores={temp.timer=1..}] run particle flame ~ ~1 ~ .05 .2 .05 .05 1 force @a[distance=..32]

execute as @e[type=hopper_minecart,tag=refinery] if score @s temp.timer_check = @s temp.timer run scoreboard players reset @s temp.timer
execute as @e[type=hopper_minecart,tag=refinery] run scoreboard players operation @s temp.timer_check = @s temp.timer