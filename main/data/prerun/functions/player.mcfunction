...
##############
	...
	execute if entity @s[scores={damage=1..}] run function prerun:player/scoreboards/damage
	...
	function prerun:player/pay/run
	function prerun:player/pln_show/run
...
##########
	...
	execute if data storage inventory baubles[0].tag{type:"fire_resistance_ring"} run function prerun:player/baubles/fire_resistance_ring
	execute unless data storage inventory baubles[0].tag{type:"fire_resistance_ring"} if data storage inventory baubles[1].tag{type:"fire_resistance_ring"} run function prerun:player/baubles/fire_resistance_ring
	
	execute if data storage inventory baubles[0].tag{type:"wither_resistance_ring"} run function prerun:player/baubles/wither_resistance_ring
	execute unless data storage inventory baubles[0].tag{type:"wither_resistance_ring"} if data storage inventory baubles[1].tag{type:"wither_resistance_ring"} run function prerun:player/baubles/wither_resistance_ring
	
	execute if data storage inventory baubles[0].tag{type:"firespark_ring"} if entity @s[nbt=!{HurtTime:0s}] run function prerun:player/baubles/firespark_ring
	execute unless data storage inventory baubles[0].tag{type:"firespark_ring"} if data storage inventory baubles[1].tag{type:"firespark_ring"} if entity @s[nbt=!{HurtTime:0s}] run function prerun:player/baubles/firespark_ring
	
	execute if data storage inventory baubles[0].tag{type:"baskalisk_heart"} if entity @s[tag=!rifting] run function prerun:player/baubles/baskalisk_heart
	execute unless data storage inventory baubles[0].tag{type:"baskalisk_heart"} if data storage inventory baubles[1].tag{type:"baskalisk_heart"} if entity @s[tag=!rifting] run function prerun:player/baubles/baskalisk_heart
	
	execute if data storage inventory baubles[0].tag{type:"slimy_eye"} run function prerun:player/baubles/slimy_eye
	execute unless data storage inventory baubles[0].tag{type:"slimy_eye"} if data storage inventory baubles[1].tag{type:"slimy_eye"} run function prerun:player/baubles/slimy_eye
	
	execute if data storage inventory baubles[0].tag{type:"ghastly_amulet"} run function prerun:player/baubles/ghastly_amulet
	execute unless data storage inventory baubles[0].tag{type:"ghastly_amulet"} if data storage inventory baubles[1].tag{type:"ghastly_amulet"} run function prerun:player/baubles/ghastly_amulet
	
	execute if data storage inventory baubles[0].tag{type:"clownfish_necklace"} run function prerun:player/baubles/clownfish_necklace/run_a
	execute unless data storage inventory baubles[0].tag{type:"clownfish_necklace"} if data storage inventory baubles[1].tag{type:"clownfish_necklace"} run function prerun:player/baubles/clownfish_necklace/run_b
	
	execute if data storage inventory baubles[0].tag{type:"ice_cube_hat"} run function prerun:player/baubles/ice_cube/run_a
	execute unless data storage inventory baubles[0].tag{type:"ice_cube_hat"} if data storage inventory baubles[1].tag{type:"ice_cube_hat"} run function prerun:player/baubles/ice_cube/run_b
...
### Armor ###
	function prerun:player/armor/run

### Random Crits ###
	function prerun:player/random_crits
	
### Temperature ###
	function prerun:player/temperature/run
...
#################
	...
	execute if entity @s[scores={kys=1..}] run function prerun:player/kys
...
###################
	...
	execute if entity @s[scores={break_item=1..}] run function prerun:player/break_item
...
### Pocket Dimensions ###
	...
	execute if entity @s[gamemode=!spectator,predicate=dimension/overworld,x=-21,y=27,z=-17,distance=..1] unless entity @s[name=!Desynq,name=!zaxaco,name=!7carmeloanthony] if block ~ ~ ~ warped_door[open=true] run function prerun:player/pocket/enter
	
	execute if entity @s[gamemode=!spectator,predicate=dimension/pocket] if block ~ ~ ~ end_gateway run function prerun:player/pocket/exit


### Myst ###
	function prerun:player/portals/run
...
#execute unless predicate clear_weather in overworld if blocks ~ ~1 ~ ~ 255 ~ ~ ~3 ~ masked run effect give @s[nbt=!{ActiveEffects:[{Id:17b}]},distance=0..] hunger 5 1 true
...
###########
	...
	effect give @s[tag=lethacus] slowness 1 0 true
	tag @s[tag=lethacus,tag=dead_one_tick] remove lethacus

	execute if entity @s[name=Desynq,scores={sneak=1}] at @e[tag=lethacus,distance=..64] run particle dust 1 0 0 1 ~ ~1 ~ .5 .5 .5 0 1 force @s
...
execute if entity @s[tag=necronoma] run function prerun:player/status_effects/run