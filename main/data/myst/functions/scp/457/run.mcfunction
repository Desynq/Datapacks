...
execute if entity @s[predicate=dimension/overworld,scores={ent.hp=150..}] run fill ~-1 ~.5 ~-1 ~1 ~.5 ~1 fire keep
...
execute if score 100 tick matches 100 if entity @a[distance=..64,gamemode=!creative,gamemode=!spectator] run summon blaze ~ ~ ~ {DeathLootTable:""}
execute if score 100 tick matches 100 if entity @a[distance=..64,gamemode=!creative,gamemode=!spectator] run summon zombified_piglin ~ ~ ~ {HandItems:[{id:golden_sword,Count:1}],DeathLootTable:"",AngerTime:2147483647}
