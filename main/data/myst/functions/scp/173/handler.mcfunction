...
execute if entity @s[nbt={ActiveEffects:[{Id:5b}]},gamemode=creative] as @a[gamemode=!spectator,gamemode=!creative,tag=!scp,tag=!173,distance=..1,scores={DeathTime=0}] run function myst:scp/173/kill
...
execute as @a[gamemode=!spectator,gamemode=!creative,scores={DeathTime=0},tag=!173,distance=..32] at @s run function myst:scp/173/ray_spawn
