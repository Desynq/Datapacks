execute if score p3_phase stage_5-1 matches 0 if entity @a[distance=..64,nbt={DeathTime:0s},tag=!dead] unless entity @e[type=wither_skeleton,tag=arch_eldritch_guardian] run function myst:mobs/stage_5-1/eldritch_eye/swap_phase
